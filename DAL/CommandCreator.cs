...
namespace SSP_1.DAL
{
    public class CommandCreator
    {
        ...
        public OleDbCommand SelectAllSpecifiedFromTable(string tableName, string specifieddictionary)
        {
            ...
            command.CommandText = $"SELECT * FROM {tableName} WHERE dictionarytype='{specifieddictionary}'";
        ...
        {
            var query = $"SELECT * FROM {taleName} WHERE {sourceLanguage}='{correctedWord}' AND dictionarytype='{specifieddictionary}'";
